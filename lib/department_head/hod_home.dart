...
import 'package:shiksha_hub/department_head/voicecollege.dart';
import 'package:shiksha_hub/chatBot/chat_bot_ai.dart';
import 'package:shiksha_hub/widgets/drawer_hod.dart';
...
class HodHome extends StatefulWidget {
  const HodHome({super.key});

  @override
  State<HodHome> createState() => _HodHomeState();
}

class _HodHomeState extends State<HodHome> with WidgetsBindingObserver {
  final user = FirebaseAuth.instance.currentUser;
  final TextEditingController _searchController = TextEditingController();
  
  Map<String, dynamic>? hodProfileData;
  ...
  Future<void> _initializeApp() async {
    ...
    await _loadHodProfile();
    
    Future.delayed(const Duration(milliseconds: 500), () {
      if (mounted) {
        ProfileCheckDialog.checkAndShowDialog(context, userType: 'department_head');
      }
    });
  }
  ...
  Future<Map<String, dynamic>?> _getHodData() async {
    if (!_hasInternetConnection) {
    ...
    try {
      ...
      final hodData = doc.data() as Map<String, dynamic>;
      final college = hodData['collegeName'];
      final branchId = hodData['branchId'];
      final branchName = hodData['branchName'];
      ...
      debugPrint('Error getting HOD data: $e');
      if (mounted) {
        setState(() {
          isLoadingData = false;
        });
  ...
  Future<void> _navigateToNotes() async {
    ...
    final hodData = await _getHodData();
    if (hodData == null || !mounted) return;

    if (hodData['college'] == null || hodData['college'].toString().trim().isEmpty) {
    ...
    if (mounted) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => SelectBranchAdmin(
            selectedCollege: hodData['college'] ?? '',
          ),
        ),
      );
    }
  ...
  Future<void> _navigateToTimeTable() async {
    ...
    if (hodData['college'] == null || hodData['college'].toString().trim().isEmpty) {
      _showErrorSnackBar('College information is required to access timetable');
      return;
    }
  ...
  final List<Map<String, dynamic>> menuItems = [
    ...
    },
    {
      ...
      "title": "Time Table",
      "description": "View & manage schedules",
    },
  ];
  ...
  if (hodProfileData == null) return const SizedBox.shrink();

  final fullName  = hodProfileData!['name'] as String? ?? 'User';
  final firstName = fullName.split(' ').first;
  final branch    = hodProfileData!['branchName'] as String? ?? '';
  final college   = hodProfileData!['collegeName'] as String? ?? '';
  final role      = hodProfileData!['role'] as String? ?? 'HOD';
  final imgUrl    = hodProfileData!['profileImageUrl'] as String? ?? '';
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              if (branch.isNotEmpty) _buildInfoChip(branch, Icons.account_tree_outlined, MediaQuery.of(context).size.width),
              const SizedBox(height: 4),
              if (college.isNotEmpty) _buildInfoChip(college, Icons.school_outlined, MediaQuery.of(context).size.width),
            ],
          ),
        ),
      ],
            ...
            SliverAppBar(
              ...
              title: Container(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.account_tree, color: Colors.indigo, size: 20),
