import 'package:shiksha_hub/department_head/d_notes/select_scheme.dart';
...
class _C {
  ...
  // Per-branch accent colours — keyword → colour, most-specific first
  static const _accents = <String, Color>{
    // CS / IT
    'AIML':                Color(0xFF0277BD), // light blue
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class SelectBranchAdmin extends StatefulWidget {
  final String selectedCollege;
  const SelectBranchAdmin({super.key, required this.selectedCollege});

  @override
  State<SelectBranchAdmin> createState() => _SelectBranchAdminState();
}

class _SelectBranchAdminState extends State<SelectBranchAdmin>
    with SingleTickerProviderStateMixin {

  // PERFORMANCE: single AnimationController (removed CPU-hungry BubblePainter)
  ...
  // Each entry: keyword → (icon, short description).
  // Ordered from most-specific to least-specific so longer matches win.
  static const List<(String, IconData, String)> _kwMap = [
    // AI / ML variants
    ...
    // Mathematics / Physics / Basic science
  ...
  /// Returns the best-matching icon for [name], or a generic school icon.
  ...
  /// Returns a short description derived from [name].
  /// - First tries the keyword map.
  /// - If unmatched and the name already looks like a full phrase (contains a
  ///   space or is longer than 6 chars), returns it as-is with " Department".
  /// - Short abbreviations (≤6 chars, no spaces) get " Department" appended.
  static String _descFor(String name) {
    final up = name.toUpperCase();
    for (final (kw, _, desc) in _kwMap) {
      if (up.contains(kw)) return desc;
    }
    // Graceful fallback: use the branch name itself
  ...
  Future<void> _loadBranches() async {
    try {
      ...
      // PERFORMANCE: 2 Firestore reads total (HOD doc + branch collection query)
      ...
      if (!hodSnap.exists) throw Exception('HOD profile not found');
      final hod = hodSnap.data()!;

      _hodBranchId = hod['branchId'] as String?;
      final collegeId   = hod['collegeId']   as String?;
      final collegeName = hod['collegeName'] as String?;
      ...
      // HOD's branch first → then A–Z
      list.sort((a, b) {
        final aHod = a['id'] == _hodBranchId;
        final bHod = b['id'] == _hodBranchId;
  ...
  void _go(String id, String name) => Navigator.of(context).push(
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => SelectSchemePage(
        branch: name,
        branchId: id,
        ...
        child: InkWell(
          ...
          child: Ink(
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isHod
                    ? _C.amber.withOpacity(0.85)
                    : _C.indigo100,
                width: isHod ? 1.8 : 1.0,
              ),
              boxShadow: [
                BoxShadow(
                  color: (isHod ? _C.amber : _C.indigo500)
                      .withOpacity(isHod ? 0.18 : 0.10),
                  blurRadius: isHod ? 18 : 10,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _PillButton(
              label: 'Try Again', icon: Icons.refresh, onTap: onRetry),
        ],
      ),
    ),
  );
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          _PillButton(
              label: 'Refresh', icon: Icons.refresh, onTap: onRetry),
        ],
      ),
    ),
  );
...
// ─────────────────────────────────────────────────────────────────────────────
//  PAGE TRANSITION EXTENSION  (kept for backward compat)
// ─────────────────────────────────────────────────────────────────────────────
