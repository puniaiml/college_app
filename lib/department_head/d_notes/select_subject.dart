...
import 'package:shiksha_hub/department_head/d_notes/select_module.dart';
...
// ─────────────────────────────────────────────────────────────────────────────
//  DESIGN TOKENS  (mirrors select_scheme / select_semester / select_section)
// ─────────────────────────────────────────────────────────────────────────────
class _C {
...
class _SelectSubjectPageState extends State<SelectSubjectPage>
  ...
  // ── Data ────────────────────────────────────────────────────────────────────
  Future<void> _loadHodCourseInfo() async {
    try {
      ...
      if (doc.exists && mounted) {
        ...
        setState(() {
          _hodCourseId   = d['courseId'];
          _hodCourseName = d['courseName'];
        });
      }
    } catch (e) {
      debugPrint('Error loading HOD course info: $e');
    }
  }
  ...
  void _goToModules(String subjectName) => Navigator.push(
    context,
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => ChapterModulePage(
  ...
  void _goToAddSubject() {
    if (_hodCourseId == null || _hodCourseName == null) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => HODSubjectManagementPage(
          schemeId:   widget.schemeId ?? '',
