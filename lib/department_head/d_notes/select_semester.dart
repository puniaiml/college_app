import 'package:shiksha_hub/department_head/d_notes/select_section.dart';
...
// ─────────────────────────────────────────────────────────────────────────────
//  DESIGN TOKENS  (mirrors select_scheme.dart / ad_branch.dart)
// ─────────────────────────────────────────────────────────────────────────────
class _C {
  ...
  // Creator-type badge colours
  static Color badgeColorFor(String? type) {
    switch (type) {
      case 'admin':           return const Color(0xFFC62828);
      case 'hod':
      case 'department_head': return const Color(0xFF4527A0);
      case 'college_staff':   return const Color(0xFF1B5E20);
      default:                return indigo700;
    }
  }
}
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class SelectSemesterPage extends StatefulWidget {
  ...
  @override
  State<SelectSemesterPage> createState() => _SelectSemesterPageState();
}

class _SelectSemesterPageState extends State<SelectSemesterPage>
  ...
  void _go(String semesterTitle) => Navigator.push(
    context,
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => SectionSelectionPage(
  ...
  void _showOptionsSheet(int index) {
    ...
    final data     = sem['data'] as Map<String, dynamic>;
    final canEdit  = _canModify(data);
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _OptionsSheet(
        title:    title,
        canEdit:  canEdit,
        onManage: () { Navigator.pop(context); _go(title); },
        onDelete: () { Navigator.pop(context); _showDeleteDialog(index, title); },
      ),
    );
  }

  bool _canModify(Map<String, dynamic> data) {
    final createdBy    = data['createdBy'];
    final currentUser  = FirebaseAuth.instance.currentUser?.uid;
    return createdBy == null || createdBy == currentUser;
  }
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              try {
                await FirebaseFirestore.instance
                    .collection('semesters')
                    .doc(_semesters[index]['id'] as String)
                    .delete();
            ...
            child: const Text('Delete',
                style: TextStyle(fontFamily: 'Poppins', color: _C.white)),
          ),
        ],
      ),
    );
  ...
  Widget _body() {
    ...
    if (_semesters.isEmpty) {
      return _EmptyView(
          branch: widget.branch, scheme: widget.scheme, onRetry: _loadSemesters);
    }
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'No semesters found for $branch under $scheme.\nUse HOD Semester Management to create them.',
            textAlign: TextAlign.center,
            style: TextStyle(
                fontFamily: 'Poppins', fontSize: 13, color: Colors.grey[600]),
          ),
