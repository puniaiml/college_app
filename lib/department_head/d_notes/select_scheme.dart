...
import 'package:shiksha_hub/department_head/d_notes/select_semester.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

// ─────────────────────────────────────────────────────────────────────────────
//  DESIGN TOKENS  (mirrors ad_branch.dart)
// ─────────────────────────────────────────────────────────────────────────────
class _C {
  ...
  // Scheme-type accent colours
  static const schemeAccents = <String, Color>{
    'admin':           Color(0xFFC62828), // deep red
    'department_head': Color(0xFF4527A0), // deep purple
    'college_staff':   Color(0xFF1B5E20), // deep green
  };

  static Color accentFor(String? type) =>
      schemeAccents[type] ?? indigo700;

  // Rotating palette for schemes that have no specific type
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class SelectSchemePage extends StatefulWidget {
  ...
  @override
  State<SelectSchemePage> createState() => _SelectSchemePageState();
}

class _SelectSchemePageState extends State<SelectSchemePage>
  ...
  Future<void> _loadSchemes() async {
    try {
      ...
      final courseId = hodSnap.data()!['courseId'] as String?;
      if (courseId == null) throw Exception('Course info missing in profile');

      // No orderBy here — we sort client-side by academic year
      ...
      for (final doc in snap.docs) {
        ...
        // Skip schemes not applicable to this branch
        if (branches != null &&
            branches.isNotEmpty &&
  ...
  void _go(Map<String, dynamic> scheme) => Navigator.of(context).push(
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => SelectSemesterPage(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        // Academic year pill (only when present)
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'No active academic schemes found for your course.\nContact the administrator or create a new scheme.',
            textAlign: TextAlign.center,
            style: TextStyle(fontFamily: 'Poppins',
                fontSize: 13, color: Colors.grey[600]),
          ),
          ...
          Row(
            ...
            children: [
              ...
              _PillButton(
                ...
                onTap: () => Get.snackbar(
                  'Info',
                  'Navigate to Scheme Management to create new schemes',
                  backgroundColor: _C.indigo500.withOpacity(0.10),
                  colorText: _C.indigo500,
                ),
              ),
            ],
          ),
