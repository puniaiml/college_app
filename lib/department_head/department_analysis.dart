...
class HodBranchDashboard extends StatefulWidget {
  const HodBranchDashboard({super.key});
  @override
  State<HodBranchDashboard> createState() => _DashState();
}

class _DashState extends State<HodBranchDashboard>
  ...
  String? _collegeId, _branchId, _branchName, _courseName, _collegeName;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final doc = await _db
          .collection('users').doc('department_head')
          .collection('data').doc(user.uid).get();
      if (!doc.exists) throw Exception('HOD data not found');
      ...
      if (!mounted) return;
      setState(() {
        final yr = byYear.entries.toList();
        _byYear = List.generate(yr.length, (i) =>
            ChartData(yr[i].key, yr[i].value.toDouble(),
        ...
        final rl = byRole.entries.toList();
        _byRole = List.generate(rl.length, (i) =>
            ChartData(rl[i].key, rl[i].value.toDouble(),
                _T.palette[i % _T.palette.length]));
      ...
      child: Scaffold(
        ...
        body: RefreshIndicator(
          ...
          child: CustomScrollView(
            // AlwaysScrollable so RefreshIndicator works even if content
            ...
            slivers: [
              ...
              SliverPadding(
                padding: EdgeInsets.fromLTRB(
                    ...
                    narrow ? 16 : 24, 60),
                sliver: SliverList(
                  // Flat list — no nested scroll, no SliverToBoxAdapter+scroll
                  delegate: SliverChildListDelegate([
                    if (_error != null)
                      _errorWidget()
                    else if (_loading)
                      ..._skeletonItems(narrow)
                    else
    ...
    return Stack(
      ...
      children: [
        ...
        Positioned(top: -40, right: -40,
            child: _Orb(color: _T.blue.withOpacity(0.22), size: 200)),
        Positioned(bottom: 10, left: 40,
            child: _Orb(color: _T.cyan.withOpacity(0.10), size: 150)),
        // Bottom fade
        Positioned(
        ...
        // Content — Positioned so it never fights for unbounded space
        Positioned(
  ...
  // ── Content Items ──────────────────────────────────────────────────────────
  // Returns a flat list — consumed by SliverChildListDelegate.
  // NO nested scrollable, NO GridView with childAspectRatio.
  ...
  Widget _statsSection(bool narrow, int cols, double sw) {
    ...
    // Wrap avoids fixed childAspectRatio — tiles size to their own content
    final tileW = cols == 3
        ? (sw - (narrow ? 32 : 48) - 24) / 3
        : (sw - (narrow ? 32 : 48) - 12) / 2;
  ...
  Widget _statTile(String label, String value, String sub,
    ...
    return Container(
      ...
      // mainAxisSize.min → column only as tall as its children
      child: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
    ...
    // fadeIn only — slideY can cause hit-test failures before first layout
    ).animate().fadeIn(
  ...
  Widget _chartCard({
    ...
    required double chartH,  // always explicit — no Expanded, no unbounded
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Fixed-height chart box — the only safe pattern inside Column
          SizedBox(
            height: chartH,
            child: Padding(
              padding: const EdgeInsets.all(10),
      ...
      series: <CartesianSeries>[
        AreaSeries<ChartData, String>(
          ...
          markerSettings: const MarkerSettings(
            ...
            color: _T.cyan,
            borderColor: _T.bgCard, borderWidth: 2,
          ),
  ...
  // 2-column layout using Row pairs — safe alternative to GridView+aspectRatio
  Widget _pieRows() {
    const h = 230.0;
  ...
  List<Widget> _skeletonItems(bool narrow) {
    Widget box(double h) => Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Shimmer.fromColors(
        baseColor: _T.bgCard,
        highlightColor: _T.bgCardAlt,
        child: Container(
          height: h,
          decoration: BoxDecoration(
            color: _T.bgCard,
