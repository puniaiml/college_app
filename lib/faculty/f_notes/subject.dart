...
import 'package:shiksha_hub/faculty/f_notes/select_module.dart';
...
// ─────────────────────────────────────────────────────────────────────────────
//  DESIGN TOKENS  (matches SelectSubjectPage exactly)
// ─────────────────────────────────────────────────────────────────────────────
class _C {
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class FacultySelectSubjectPage extends StatefulWidget {
  ...
  const FacultySelectSubjectPage({
    super.key,
  ...
  @override
  State<FacultySelectSubjectPage> createState() => _FacultySelectSubjectPageState();
}

class _FacultySelectSubjectPageState extends State<FacultySelectSubjectPage>
  ...
  // ── Data ────────────────────────────────────────────────────────────────────
  Future<void> _loadFacultyCourseInfo() async {
    try {
      ...
      if (doc.exists && mounted) {
        ...
        setState(() {
          _facultyCourseId   = d['courseId'];
          _facultyCourseName = d['courseName'];
        });
      }
    } catch (e) {
      debugPrint('Error loading faculty course info: $e');
    }
  }
  ...
  void _goToModules(String subjectName) => Navigator.push(
    context,
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => FacultyChapterModulePage(
  ...
  void _goToAddSubject() {
    if (_facultyCourseId == null || _facultyCourseName == null) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Loading course information…'),
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => FacultySubjectManagementPage(
          schemeId:   widget.schemeId ?? '',
          schemeName: widget.scheme,
...
// ─────────────────────────────────────────────────────────────────────────────
//  SUBJECT CARD  (with optional creator badge — faculty-specific addition)
// ─────────────────────────────────────────────────────────────────────────────
