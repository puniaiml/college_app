...
import 'dart:io';
import 'package:shiksha_hub/faculty/f_notes/view_pdf.dart';
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class FacultyUploadPdfPage extends StatefulWidget {
  ...
  @override
  State<FacultyUploadPdfPage> createState() => _FacultyUploadPdfPageState();
}

class _FacultyUploadPdfPageState extends State<FacultyUploadPdfPage>
  ...
  Future<void> _selectFile() async {
    try {
      ...
      if (file.size > maxSize) {
        _snack(
          'File too large (max 50 MB): ${(file.size / 1024 / 1024).toStringAsFixed(1)} MB',
          Colors.redAccent,
        );
        return;
      }
      if (file.size == 0) {
        _snack('Selected file is empty', Colors.redAccent);
        return;
      }

      setState(() => _pickedFile = file);
    } catch (e) {
      _snack('File selection failed: $e', Colors.redAccent);
    }
  }
  ...
  Future<void> _uploadFile() async {
    ...
    if (filePath == null) {
      _snack('Invalid file path — please reselect the file', Colors.redAccent);
      setState(() => _pickedFile = null);
      return;
    }

    if (!File(filePath).existsSync()) {
      _snack('File no longer exists — please reselect', Colors.redAccent);
      setState(() => _pickedFile = null);
      return;
    }
    ...
    if (user == null) {
      _snack('You must be logged in to upload', Colors.redAccent);
      return;
    }
    ...
    try {
      ...
      _snack('Upload failed: ${e.message ?? e.code}', Colors.redAccent);
      setState(() => _uploadTask = null);
    } on SocketException catch (e) {
      print('Network error: $e');
      _snack('Network error — check your connection', Colors.redAccent);
      setState(() => _uploadTask = null);
    } catch (e) {
  ...
  void _goViewPdf() => Navigator.push(
    context,
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => FacultyViewPdfPage(
                  ...
                  ..._actions.asMap().entries.map((e) {
                    ...
                    return RepaintBoundary(
                      child: AnimatedBuilder(
                        ...
                        child: _ActionCard(item: item, index: i),
                      ),
                    );
                  }),
                ],
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: uploadTask != null ? null : onClear,
              ),
            ],
          ),
          if (uploadTask != null) ...[
