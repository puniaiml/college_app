...
import 'package:shiksha_hub/widgets/drawer_user.dart';
...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  Future<void> _initializeApp() async {
    ...
    Future.delayed(const Duration(milliseconds: 500), () {
      if (mounted) {
        ProfileCheckDialog.checkAndShowDialog(context, userType: 'student');
      }
    });
  }
  ...
  bool _validateProfileData(Map<String, dynamic> data) {
    final requiredFields = ['email', 'usn'];
  ...
  Future<Map<String, dynamic>?> _getStudentData() async {
    ...
    try {
      ...
      if (!doc.exists) {
        if (mounted) {
          _showErrorSnackBar('Profile not found. Please update your profile.');
        }
        return null;
      }

      final studentData = doc.data() as Map<String, dynamic>;
      final college = studentData['collegeName'] ?? studentData['college'];
      ...
      return {
        'college': college,
        'course': studentData['courseName'] ?? studentData['course'],
        'branch': studentData['branchName'] ?? studentData['branch'],
  ...
  Future<void> _navigateToNotes() async {
    ...
    if (studentData == null || !mounted) return;

    if (studentData['college'] == null || studentData['college'].toString().trim().isEmpty) {
    ...
    if (mounted) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => BranchStudent(
            selectedCollege: studentData['college'] ?? '',
          ),
        ),
      );
    }
  ...
  Future<void> _navigateToTimeTable() async {
    ...
    try {
      ...
      final branchId = studentData['branchId'] as String?;
      final branchName = studentData['branchName'] as String?;
      final college = studentData['collegeName'] ?? studentData['college'];
  ...
  if (studentProfileData == null) return const SizedBox.shrink();

  String fullName = studentProfileData!['fullName'] ??
  ...
  final usn       = studentProfileData!['usn'] as String? ?? '';
  final branch    = studentProfileData!['branchName'] as String? ?? '';
  final college   = studentProfileData!['collegeName'] as String? ?? '';
  final status    = studentProfileData!['accountStatus'] as String? ?? '';
  final imgUrl    = studentProfileData!['profileImageUrl'] as String? ?? '';
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              if (usn.isNotEmpty)
                _buildInfoChip(usn, Icons.badge_outlined,
