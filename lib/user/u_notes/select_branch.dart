import 'package:shiksha_hub/user/u_notes/scheme_selection.dart';
...
// ─────────────────────────────────────────────────────────────────────────────
//  DESIGN TOKENS  (mirrors SelectBranchAdmin exactly)
// ─────────────────────────────────────────────────────────────────────────────
class _C {
...
//  MAIN WIDGET
// ─────────────────────────────────────────────────────────────────────────────
class BranchStudent extends StatefulWidget {
  final String selectedCollege;
  const BranchStudent({super.key, required this.selectedCollege});

  @override
  State<BranchStudent> createState() => _BranchStudentState();
}

class _BranchStudentState extends State<BranchStudent>
  ...
  Future<void> _loadBranches() async {
    try {
      ...
      if (!studentSnap.exists) throw Exception('Student data not found');
      final studentData = studentSnap.data()!;

      _studentBranchId = studentData['branchId'] as String?;

      final collegeId   = studentData['collegeId']   as String?;
      final collegeName = studentData['collegeName'] as String?;
      ...
      // Student's branch first → then A–Z
      list.sort((a, b) {
        final aMine = a['id'] == _studentBranchId;
        final bMine = b['id'] == _studentBranchId;
  ...
  void _go(String id, String name) => Navigator.of(context).push(
    PageRouteBuilder(
      pageBuilder: (_, __, ___) => StudentSelectSchemePage(
        branch:          name,
        branchId:        id,
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isMine
                    ...
                    : _C.indigo100,
                width: isMine ? 1.8 : 1.0,
              ),
              boxShadow: [
                BoxShadow(
                  color: (isMine ? _C.amber : accent)
                      .withOpacity(isMine ? 0.18 : 0.10),
                  blurRadius: isMine ? 18 : 10,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'No active branches found for your college.\nPlease contact the administrator.',
            textAlign: TextAlign.center,
            style: TextStyle(
                fontFamily: 'Poppins',
                fontSize:   13,
